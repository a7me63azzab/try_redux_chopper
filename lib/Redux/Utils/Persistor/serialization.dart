...
class CustomAppStateSerializer<T> implements StateSerializer<T> {
  /// Turns the dynamic [json] (can be null) to [T]
  final T? Function(String? json) decoder;

  CustomAppStateSerializer(this.decoder);

  @override
  T? decode(Uint8List? data) {
    print("THIS IS CURRENTLY WORKING $data");
    return decoder(data != null ? json.decode(uint8ListToString(data)!) : null);
  }
